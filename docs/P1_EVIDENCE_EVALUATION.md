...
## Status: built — see `docs/EVALUATION_HARNESS.md`

The harness above is built: `apps/api/src/evaluation/` (70 gold cases across all 12 categories, `npm run evaluate` to run it, excluded from the default `npm test` merge gate). Baseline recall/precision per category, the confirmed reproductions of every finding above, and several new findings the larger set surfaced that this one-shot round couldn't (pressure ulcers are currently unmatchable at all — a more severe version of the numeric-staging gap; DVT laterality doesn't suppress the opposite side; negated qualifiers inside a "with X" structure and unconfirmed differentials both still fire) are recorded there. **No fixes from Section 3 above have been applied yet** — per explicit instruction, they ship one at a time against this baseline (`baseline → fix → evaluate → fix → evaluate`), not all at once.
