...
### CodingService data-integrity gaps: mixed codeSystem, duplicates, POA, and transaction atomicity

The workflow-ownership arc answered "who may mutate an encounter?" This phase answers the next question: **can a permitted mutation leave the database in a partially corrupted state if something inside the operation fails?** — plus the remaining payload-level invariants `saveDraft`/`finalize` hadn't been tested against yet (code validity, billability, and the principal-diagnosis invariant were already covered by the earlier P0 pass).

**New payload-level coverage (7 tests):**
- Mismatched `codeSystem` — a diagnosis submitted as `ICD-10-PCS` or a procedure submitted as `ICD-10-CM` is rejected (Zod's `z.literal` on each schema makes this a shape-validation rejection, not a business-rule check — confirmed by reading `coding-decision.ts`, not assumed).
- POA (`presentOnAdmission`) is a required boolean, not defaulted or coerced — missing entirely or given as a non-boolean (`"yes"`) is rejected.
- Duplicate procedure codes: pinned as **current behavior, not asserted as correct** — same gap as the already-documented duplicate-diagnosis-code behavior. No dedup check exists in `CodingDecisionSchema` or `CodingService` for either diagnoses or procedures.
- **No "automatic principal promotion after removal" logic exists anywhere in the backend**, confirmed by reading `CodingService` and `CodingDecisionSchema` end to end. `saveDraft()` re-validates "exactly one principal" from scratch on every call rather than repairing a submission that removed the principal without designating a new one — the existing "rejects zero principal diagnoses" test **is** that invariant's enforcement. Any promotion UX is therefore a frontend-only concern with no backend state to keep in sync, because the backend never partially accepts an invalid submission in the first place.

**Transaction atomicity (3 tests, all teeth-proofed against the real transaction wrapping, not a contrived stand-in):** rather than assume `saveDraft()`'s and `finalize()`'s existing `$transaction(async (tx) => {...})` blocks are atomic, each was forced to fail partway through — using a Prisma `$extends` query interceptor that throws on `auditEntry.create` (the last write before each transaction's closing `encounter.update`, so a clean rollback here proves the whole block is atomic, not just the writes before the interception point) — verified first as a standalone experiment, then written up as regression tests. All three confirmed the existing code is already correctly atomic: `saveDraft()` leaves no coding decision and no status change on a failed first save, and leaves the prior decision/status completely untouched on a failed update; `finalize()` leaves `finalizedAt`/`msDrg`/status/the audit trail completely untouched, with no audit entry beyond what existed before the call and no `QaReview` row (since `maybeSampleForReview()` only runs after the transaction commits, and never got the chance to). Teeth confirmed by temporarily replacing each `this.prisma.$transaction(async (tx) => {...})` with a non-transactional `(async (tx) => {...})(this.prisma)` — same code, no atomicity — and observing all three tests fail with exactly the partial-write symptom they're designed to catch, then restoring the fix and confirming they pass again.

**A genuine finding, reported rather than silently fixed (1 test, documenting current behavior — same discipline as the open-query business rule):** `finalize()`'s call to `qaService.maybeSampleForReview()` runs *after* its own transaction has already committed, and isn't wrapped in try/catch. Direct experiment (forcing `qaReview.create` to throw, with `Math.random` mocked to force a sampling hit) confirmed: the sampling call's own internal transaction rolls back cleanly on failure (no orphaned `QaReview` row — the database is never left inconsistent), **but the exception still propagates out of `finalize()` to the caller**, even though the finalize itself — `finalizedAt`, `msDrg`, encounter status, and the `FINALIZE` audit entry — already committed successfully and is not rolled back. The caller receives an error response describing an operation that, in fact, already succeeded. This is a real gap in the API contract, not a data-corruption risk, and is reported here as an open product question rather than fixed: should `finalize()` catch and log a sampling failure instead of surfacing it as the request's own failure? That depends on whether the client needs to know sampling didn't run — a decision, not an obvious bug fix.

Full backend suite: 125/125 passing (up from 115). Typecheck clean. No leftover test data.

Files: [apps/api/src/modules/coding/coding.service.ts](../apps/api/src/modules/coding/coding.service.ts), [apps/api/src/modules/coding/coding.service.spec.ts](../apps/api/src/modules/coding/coding.service.spec.ts)

### A fourth instance of the lock-bypass bug class, found by cross-service Query + QA integration testing

Having tested `CodingService`, `QueriesService`, and `QaService` largely in isolation, this phase specifically targeted the interactions *between* Query and QA ownership — the next boundary after payload-level data integrity. Per the same "derive from the actual implementation, don't assume" discipline used throughout this pass, the question was: can query and QA ownership ever produce a contradictory state?

**Direct experiment surfaced a real, previously-unknown bug.** Under strictly sequential API use, a `SENT`/`RESPONDED` query can never coexist with a `FINALIZED`/`QA_REVIEW` encounter, because `finalize()`'s own open-query business rule refuses to finalize while any query is still open (see "Encounter ownership / locked-state audit" above) — so it looked, at first, like `respond()`/`resolve()` not checking encounter status was safe by construction. But that invariant is a **TOCTOU race, not a guarantee**: `create()` and `send()` both only check the encounter's status at the moment they run, so both can pass while the encounter is still `IN_PROGRESS`, and `finalize()`'s own transaction can commit around them — leaving a `SENT` query on an encounter that finishes finalizing (and is then possibly QA-sampled) with the query still open. Rather than reason about the race abstractly, the resulting *state* was constructed directly and fed to the real service methods: with a `SENT` query and the encounter forced into `QA_REVIEW`, `respond()` succeeded unconditionally, and `resolve()` went on to **silently overwrite `QA_REVIEW` back to `IN_PROGRESS`** — discarding the auditor's active review out from under them. Exactly the same failure mode as the three lock-bypass bugs already found and fixed (`CodingService.finalize()`, `CodingService.saveDraft()`, `QueriesService.create()`/`send()`), just reachable through the two remaining unguarded mutating methods on `QueriesService`.

Fixed identically to the other three: `respond()` and `resolve()` now reject when `query.encounter.status` is `FINALIZED` or `QA_REVIEW`, completing the guard across all four of `QueriesService`'s mutating methods. 6 new regression tests added to `queries.service.spec.ts` (2 per method × FINALIZED/QA_REVIEW, plus verifying the query and encounter are both left completely untouched on rejection). Teeth-proofed by temporarily disabling each new guard (`if (false)`) and confirming both the intended rejection and the original corruption (`resolve()` visibly returning `status: "RESOLVED"` with the encounter still forced to `QA_REVIEW`) reproduce exactly, then restoring and confirming green again.

**A new cross-service integration test** (`query-qa-integration.spec.ts`) exercises the case none of the per-service suites cover: a query raised, answered, and resolved *entirely within* the window between a QA return and the next finalize — not just "return, then recode, then finalize" (already covered by `qa.service.spec.ts`'s cross-module lifecycle test), but "return, then a whole new CDI query cycle, then finalize." Confirms the open-query business rule applies identically the second time through the loop, and that the original `RETURNED` review's history and the full audit trail both survive intact.

The remaining items from this phase's scope — multiple simultaneous queries interacting with finalize eligibility, and retry/idempotency of every Query/QA transition (send/respond/resolve twice, approve/return twice) — were already covered by the existing per-service suites (`queries.service.spec.ts`'s "multiple queries on one encounter" and "illegal transitions are rejected" blocks, `qa.service.spec.ts`'s "illegal transitions are rejected" block, and `coding.service.spec.ts`'s "one of several queries still open" business-rule test); confirmed by review rather than duplicated.

Full backend suite: 130/130 passing (up from 125). Typecheck clean. No leftover test data.

Files: [apps/api/src/modules/queries/queries.service.ts](../apps/api/src/modules/queries/queries.service.ts), [apps/api/src/modules/queries/queries.service.spec.ts](../apps/api/src/modules/queries/queries.service.spec.ts), [apps/api/src/test-support/query-qa-integration.spec.ts](../apps/api/src/test-support/query-qa-integration.spec.ts)
